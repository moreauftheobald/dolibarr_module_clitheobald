...
$hookmanager->initHooks(array('clitheobaldlist'));
...
llxHeader('', $langs->trans('CliTheobaldList'), '', '');

//$type = GETPOST('type');
//if (empty($user->rights->clitheobald->all->read)) $type = 'mine';
...
$sql.= ' FROM '.MAIN_DB_PREFIX.'clitheobald t ';

if (!empty($object->isextrafieldmanaged))
{
    $sql.= ' LEFT JOIN '.MAIN_DB_PREFIX.'clitheobald_extrafields et ON (et.fk_object = t.rowid)';
}

$sql.= ' WHERE 1=1';
//$sql.= ' AND t.entity IN ('.getEntity('CliTheobald', 1).')';
...
$formcore = new TFormCore($_SERVER['PHP_SELF'], 'form_list_clitheobald', 'GET');
...
$listViewConfig = array(
	...
	,'list' => array(
		'title' => $langs->trans('CliTheobaldList')
		,'image' => 'title_generic.png'
		,'picto_precedent' => '<'
		,'picto_suivant' => '>'
		,'noheader' => 0
		,'messageNothing' => $langs->trans('NoCliTheobald')
	...
	,'search' => array(
		...
		,'status' => array('search_type' => CliTheobald::$TStatus, 'to_translate' => true) // select html, la clé = le status de l'objet, 'to_translate' à true si nécessaire
	)
	,'translate' => array()
	,'hide' => array(
