...
$hookmanager->initHooks(array('ordersupplierdispatch', 'ordersupplierdispatchtheobald'));
			...
			while ($i < $num) {
				...
				if (!$objp->fk_product > 0) {
					...
					if ($remaintodispatch || empty($conf->global->SUPPLIER_ORDER_DISABLE_STOCK_DISPATCH_WHEN_TOTAL_REACHED)) {
						...
						$dPWarehouse = $dProductWarehouse->fetchWarehouse($objp->fk_product ,$object->entity);
						if($dPWarehouse){
							$objp->fk_default_warehouse = $dProductWarehouse->fk_warehouse;
						}
						/**
						 * ===========---------- END THEOBALD ----------===========
						 */
